...
/*   Updated: 2019/10/19 18:23:40 by gbeqqo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_cropw(t_token *list)
{
	while (list)
	{
		if (list->type == 6) // <<
		{
			if (list->next)
			{
				ft_putendl(list->next->buf);
				return (list->next->buf);
			}
		}
		list = list->next;
	}
...
void	heredoc_output(char *buf)
{
	...
	ft_bzero(g_shell->main_cmnd, MAXLINE);
    heredoc_readline(buf);
...
int		execute_start(t_tree *ast, int in, int out)
{
	...
	in = get_redirections(ast, in, 6, 0);
...
void	execute_right_cmd(t_tree *ast, int in, int out, int temp)
{
	...
	else
	{
		...
		&& ast->parent->parent->type != 1))
			waitpid(pid, NULL, 0);                                //почему мне было необходимо тут ждать только в таком случае
		close_fds(out, temp);
	}
}
...
int		execute_right(t_tree *ast, int in, int out, int temp)
{
	...
	temp = get_redirections(ast, temp, 6, 1);
	out = get_redirections(ast, out, 3, 1);
	create_argv(ast);
	if (argv_checker(ast->argv) == 1)
	{
...
//добавить ошибку, что не откуда читать