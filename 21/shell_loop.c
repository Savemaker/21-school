...
/*   Updated: 2019/10/23 16:25:44 by gbeqqo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//
//
//void autocom_addpath(t_autocom **list, char *clear_path, char *request_path)
//{
//    char *full_path;
//    DIR *dir;
//    struct dirent *dir_ptr;
...
//
//	autocom_addpath(list, clear_path, request_path + 1);
//
...
////		autocom_addpath(list, bin_path, request, 0, 1);/!use
//		free(bin_path);
//		i++;
//	}
//	if (all_bins)
...
////	autocom_addpath(list, ".", request, 1, 0); //!use
//
//
//
//	g_shell->auto_quantity = autocom_lstsize(*list);
...
{
	if (g_shell->inputmode == AUTOCOM_INPUT && buf[0] != 9)
	{
...
//	{
	...
	if (buff[0] == 29 && g_shell->emacs_on == 1)
	{
		g_shell->emacs_off = 1;
		g_shell->emacs_on = 0;
	}
	else if (buff[0] == 29 && g_shell->emacs_off == 1)
	{
		g_shell->emacs_off = 0;
		g_shell->emacs_on = 1;
	}
	if (buff[0] == 23 && g_shell->emacs_on == 1)
	{
		g_shell->emacs_on = 1;
		g_shell->emacs_off = 0;
		*line = ft_strdup("emacs");
		print_line(g_shell->input, 1);
		return (1);
	}
...
void readline_cmnd(char **line, int prompt_size)
{
	...
	while (1)
	{
		...
		if (buff[0] == 4 && g_shell->inputmode == HEREDOC_INPUT)
		{
			ft_putstr("\n");
			return ;
		}
...
// int display_added_prompt_heredoc()
// {
...
// void heredoc_readline(char *stop_word)
// {
//     char *line;
...
//     readline_cmnd(&line, display_added_prompt_heredoc());
//     if (ft_strcmp(line, stop_word))
//     {
//         g_shell->main_cmnd = ft_strcat(g_shell->main_cmnd, line);
//         g_shell->main_cmnd = ft_strcat(g_shell->main_cmnd,"\n");
// //        g_shell->main_cmnd = ft_strcat(g_shell->main_cmnd,"\n");
//         heredoc_readline(stop_word);
//     }
//     reset_input_tmode();
// }
...
// 	g_shell->inputmode = HEREDOC_INPUT;
// 	cropw = ft_memalloc(MAXLINE);
// 	word = ft_strrchr(str, '<');
// 	word += 2;
// 	int i = 0;
// 	while (!ft_isspace(word[i]) && word[i] != ';')
// 		i++;
//     ft_strncpy(cropw, word, i);
//     heredoc_readline(cropw);
//     if ((fd = open(".21sh_heredoc", (O_RDWR|O_CREAT|O_TRUNC), (S_IREAD|S_IWRITE))) == -1)
//         perror_cmnd(SHELLNAME, NULL, OPENERR);
//     ft_putstr_fd(g_shell->main_cmnd, fd);
//     if (close(fd) == -1)
//         perror_cmnd(SHELLNAME, NULL, CLOSEERR);
//     free(cropw);
// }
...
int display_added_prompt_pipe()
{
    ft_putstr_fd(BWHITE, STDOUT_FILENO);
    ft_putstr_fd("pipec> ", STDOUT_FILENO);
    ft_putstr_fd(RESET, STDOUT_FILENO);
    return (7);
}
...
void pipe_readline()
{
    ...
    set_input_tmode();
    line = NULL;
    readline_cmnd(&line, display_added_prompt_pipe());
    g_shell->main_cmnd = ft_strcat(g_shell->main_cmnd, line);
    if (check_pipe(line))
    {
        pipe_readline();
...
void pipe_output(char *str)
{


    g_shell->main_cmnd = ft_strcpy(g_shell->main_cmnd, str);
    pipe_readline();
}
