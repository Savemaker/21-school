...
char	*dir_autocom(t_autocom **list, char *request)
{
	...
	//from system
	all_bins = ft_strsplit(ft_getenv("PATH", g_my_env), ':');
	i = 0;
	while (all_bins[i])
	{
...
void tilde_to_path(char *homepath, char **request)
{
	...
	homepath = ft_getenv("HOME", g_my_env);
...
//
	...
	{
		reassign = 1;
		tilde_to_path(ft_getenv("HOME", g_my_env), &request);
	}
	...
	if (reassign && response)
		response = path_to_tilde(ft_getenv("HOME", g_my_env), response);
