...
void init_term(void)
{
	...
	if ((term_type = ft_getenv("TERM", g_my_env)))
	{
...
//	char **prevcmnds;
	...
	char *path;
	if ((path = ft_getenv("PWD", g_my_env)))
	...
	if ((histsize = ft_getenv("HISTSIZE", g_my_env)))
		history->size = ft_atoi(histsize);
	else
		history->size = HISTSIZE;
	if ((histfilesize = ft_getenv("HISTFILESIZE", g_my_env)))
